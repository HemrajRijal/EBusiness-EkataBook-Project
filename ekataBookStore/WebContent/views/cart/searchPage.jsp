...
function searchBookDetails()
{
	try
	{
		...
		var aonly="";
	var atitle="";
	var authoronly=$('#author').val();
		var authorortitle=$('#search').val();
		if(authoronly == "" || authoronly ==null )
			{
			  aonly="";
			}
		else
		...
		if(authorortitle == "" || authorortitle == null)
			{
			  atitle="";
			}
		else
			{
			  atitle=authorortitle;
			}
		if(aonly == "" && atitle == "")
			{
			 $('#ajaxResponse').text("Please fill the Required Search Criterias");
			}
		else
			{
			   ...
			   $.ajax({
				     ...
				     data: {aonly:aonly,atitle:atitle},
				     dataType: 'json',
				     success: function (data) {
				    	 try
						  {
						    var tbl_body = "<table border='1' cellpadding='6'> <thead> <th></th><th>TITLE</th><th>Author</th><th>Price</th><th>Book Image</th><th>Qty</th></thead>";
						    var odd_even = false;
						    var tbl_row = "";
						    //alert("outside 1");
						    $.each(data, function() {
						      ...
						      //  alert("inside 1");
						       ...
						       // 	  alert("inside 2");
						        	if(k == "isbn")
						            	{
						            	  isbnval=v;
						            	  tbl_row +="<td><input type='checkbox' name='isbn' value='" + isbnval + "' /></td>";
						            	}
						        	if(k=="price")
						        		{
						        		 tbl_row +="<td><input type='text' size='3' value='"+v+"' id='price" + isbnval + "'  readonly /></td>";
						        		}
						        	if(k != "bookimage" && k!="isbn" && k!="quantity" && k!="image" && k!="price" )
						            	{
						        	tbl_row += "<td>"+v+"</td>";
						            	}
						            else
						         ...
						         tbl_row +="<td><input type='text' size='3' value='1' id='qty" + isbnval + "' /></td>";
						    ...
						    tbl_row ="<td><input type='submit' value='Add to Cart' name='submitbtn' id='addtocartbtn'></td>";
						    tbl_row +="<td><input type='submit' value='CLEAR' name='clearbtn' id='clearbtn'></td>";
				     ...
				     {
				    	 //var responseText = jQuery.parseJSatON(xhr.responseText);
				    	// alert(xhr.responseText.StackTrace);
    ...
    <div class="container">
        <div class="header">
            <div class="logo">
                <a href="/ekataBookStore/CartController?action=displayMainPage"><img src="views/images/logo.png" alt=""/> </a>
            </div>
            <!-- start header_right -->
            <div class="header_right">
            <div class="create_btn">
                <a class="arrow"  href="#">create account <img src="views/images/right_arrow.png" alt=""/>  </a>
            </div>
...
{
%>
<option value="<%=ids.get(i) %>"> <%=ids.get(i) %></option>
<%
}
%>
...
<tr>
<td>Select Book Title or Author</td><td><input type="text" name="search" id="search"></td>
</tr>
<tr>
<td><input type="submit" value="SEARCH" onclick="searchBookDetails();"></td><td><input type="submit" value="CLEAR" ></td>
</tr>
</table>
<fieldset>
<legend align="center">Main Table </legend>
